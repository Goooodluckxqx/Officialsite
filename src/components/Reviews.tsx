...
export default function Reviews() {
  return (
    <section id="reviews" className="py-20 lg:py-28 bg-chocolate text-cream">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <div className="mt-4 flex items-center justify-center gap-2">
            <div className="flex">
              {[1, 2, 3, 4, 5].map((star) => (
                <Star key={star} className="w-5 h-5 fill-amber text-amber" />
              ))}
            </div>
            <span className="text-2xl font-bold text-milktea">4.9</span>
            <span className="text-greige">/5.0</span>
          </div>
          <p className="text-sm text-greige mt-2">Over 10,000 users have chosen NazzleNest</p>
        </motion.div>
        ...
        <div className="grid sm:grid-cols-2 gap-6">
          {reviews.map((review, index) => (
            <motion.div
              ...
              className="bg-chocolate-light/50 backdrop-blur-sm rounded-2xl p-6 border border-cream/10"
            >
              <Quote className="w-8 h-8 text-lavender mb-4" />
              
              <h3 className="font-medium text-lg mb-3">{review.title}</h3>
              <p className="text-cream/70 text-sm leading-relaxed mb-6">{review.text}</p>
              
              <div className="flex items-center justify-between pt-4 border-t border-cream/10">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-milktea rounded-full flex items-center justify-center text-cream font-medium text-sm">
                    {review.author.charAt(0)}
                  </div>
                  <div>
                    <p className="font-medium text-sm">{review.author}</p>
                    <p className="text-xs text-greige">{review.product}</p>
                  </div>
                </div>
                <div className="flex">
